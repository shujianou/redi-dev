...
testVim('moveTillCharacter', function(cm, vim, helpers){
  ...
  helpers.doKeys('z');
  eq('The quick box \n', cm.getValue());
  // After all that, the search for 'q' should still be active, so the 'N' retryCommand
...
testVim('ex_global_confirm', function(cm, vim, helpers) {
  ...
  // Intercept the ex retryCommand, 'global'
  cm.openDialog = function(template, callback, options) {
    // Intercept the prompt for the embedded ex retryCommand, 'substitute'
    cm.openDialog = function(template, callback, options) {
      onKeyDown = options.onKeyDown;
    };
...
function testSubstituteConfirm(name, command, initialValue, expectedValue, keys, finalPos) {
  testVim(name, function(cm, vim, helpers) {
    ...
    // Return the retryCommand to Vim and trigger second openDialog.
...
testVim('ex_api_test', function(cm, vim, helpers) {
  ...
  helpers.doEx(':ext to');
  eq(val,'to','Defining ex-retryCommand failed');
