...
(function($){
    ...
    InitForm.prototype = {
            ...
            init: function() {
                ...
                //autoFillListctrlListener(this.setting.userId, this.setting.orgId);
            },
            /**
             * 控制表单字段
             */
...
function autoFillListctrlListener(userId, orgId) {
    $(".cnoj-auto-fill-listctrl").each(function () {
        ...
        if(utils.isNotEmpty(relateField)) {
            $("select[name=" + relateField + ']').on('change', function () {
                _fillValue($this, userId, orgId, $(this).val());
            });
        } else {
    ...
    function _fillValue($element, userId, orgId, value) {
        var url = $element.data('fill-url');
        if(utils.isEmpty(url)) {
            return false;
        }
        if(utils.isNotEmpty(value)) {
            url = utils.handleUrl(url);
            if(url.indexOf('/query/') >= 0) {
                url += "paramName=value&paramValue="+value;
            } else {
                url += "value="+value;
            }
        }
