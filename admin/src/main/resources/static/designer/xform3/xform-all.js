...
xf.GridSection.prototype.render = function() {
	...
	var html = '<table class="xf-table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">'
		+ '<tbody>';
...
xf.field.Radio.prototype.updateItems = function(value) {
	...
	for (var i = 0; i < array.length; i++) {
		var item = array[i];
		html += '<label class="radio-inline" style="padding-top:5px;padding-bottom:5px;">';
...
xf.field.DatePicker.prototype.updateName = function(value) {
	...
	parentNode.innerHTML =
	  ...
	  + '  <input type="text" name="' + this.name + '" value="' + (this.value ? this.value : '') + '" '
	  + (this.readOnly ? ' readOnly' : '')
	  + (this.required ? ' required="true" class="form-control required"' : '')
